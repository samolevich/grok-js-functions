...
302\. Объявите функцию `foo`. Внутри объявите переменную `x` со значением `42` и вложенную функцию `bar`. Внутри функции `bar` объявите переменную `x` со значением `7`. Вызовите функцию `bar` внутри функции `foo`. Функция `foo` должна возвращать `x`. Вызовите `foo`. Чему равно значение `y`? Почему?
