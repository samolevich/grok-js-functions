...
302\. Объявите функцию `foo`. Внутри объявите переменную `x` со значением `42` и вложенную функцию `bar`. Внутри функции `bar` объявите переменную `x`, которая удваивает значение переменной `x`. Вызовите функцию `bar` внутри функции `foo`. Вызовите `foo`. Чему равно значение `y`? Почему?
